...
echo "Starting A100 4-domain validation at $(date -Iseconds)"

pkill -f hcp-ringattn-rust 2>/dev/null || true
sleep 2

rm -f /tmp/a100_4d_coord.pid
nohup ./rust/target/release/hcp-ringattn-rust --distributed-role coordinator \
  ...
  --model-dir "$MODEL_DIR" --prompt "$PROMPT" \
  --max-tokens $MAX_TOKENS --temperature $TEMP > /tmp/a100_4d_coord.log 2>&1 &
COORD_PID=$!
echo $COORD_PID > /tmp/a100_4d_coord.pid
