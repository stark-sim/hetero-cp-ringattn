#!/bin/bash
# Cross-node HTTP API end-to-end test.
# Topology: Mac MPS (coordinator + worker 0) + white RTX 4090 CUDA (worker 1)
# Coordinator runs in HTTP mode; test curls /health and /v1/completions.
...
RUN_ID="cross-node-http-$(date +%Y%m%d-%H%M%S)"
...
echo "=== Cross-Node HTTP API E2E Test ==="
echo "RUN_ID=${RUN_ID}"
...
# Read first 100 chars from prompt file as the prompt text
...
if [ -n "$GENERATED" ] && [ -z "$ERROR" ]; then
    echo "=== CROSS-NODE HTTP API E2E TEST PASSED ==="
    echo "Reports: ${REPORT_DIR}"
    exit 0
else
    echo "=== CROSS-NODE HTTP API E2E TEST FAILED ==="
