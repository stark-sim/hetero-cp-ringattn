#!/bin/bash
# Local VllmWorkerBackend mock E2E test.
# Launches coordinator (HTTP mode) + 1-domain mock worker, then curls the API.
# No real model needed — uses HCP_PYTHON_BACKEND_TYPE=mock.
...
echo "=== VllmWorkerBackend Mock Local E2E Test ==="
...
# Use mock backend so no vLLM/transformers installation needed
export HCP_PYTHON_BACKEND_TYPE=mock
export HCP_MOCK_VOCAB_SIZE=151936
export HCP_MOCK_NUM_LAYERS=24
...
"${BINARY}" --distributed-role coordinator \
    ...
    > /tmp/hcp_vllm_mock_coord.log 2>&1 &
COORD_PID=$!
echo "Coordinator PID: ${COORD_PID}"
sleep 2

# Launch Worker 0 (domain 0, vllm backend, single domain — peer points to self)
echo "=== Launching Worker 0 (vllm backend, mock) ==="
"${BINARY}" --distributed-role worker \
    ...
    --num-domains 1 \
    > /tmp/hcp_vllm_mock_w0.log 2>&1 &
W0_PID=$!
echo "Worker 0 PID: ${W0_PID}"

# Wait for worker to connect
echo "Waiting 5s for worker to connect..."
sleep 5
...
if [ -n "$GENERATED" ] && [ "$GENERATED" != "[error:"* ]; then
    echo "=== VLLM MOCK E2E TEST PASSED ==="
else
    echo "=== VLLM MOCK E2E TEST FAILED ==="
    echo "--- Coordinator log ---"
    tail -20 /tmp/hcp_vllm_mock_coord.log || true
    echo "--- Worker 0 log ---"
    tail -20 /tmp/hcp_vllm_mock_w0.log || true
    exit 1
fi
