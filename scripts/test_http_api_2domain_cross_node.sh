#!/bin/bash
# Cross-node 2-domain HTTP API end-to-end test.
...
RUN_ID="cross-node-http-2domain-$(date +%Y%m%d-%H%M%S)"
...
echo "=== Cross-Node 2-Domain HTTP API E2E Test ==="
echo "RUN_ID=${RUN_ID}"
...
if [ "${DRY_RUN:-}" = "1" ]; then
    echo "=== DRY RUN CONFIG ==="
    echo "  Mac    (d0): ${MAC_ADDR}:${W0_PORT}  -> next=${PEARL_ADDR}:${W1_PORT}"
    echo "  Pearl  (d1): ${PEARL_ADDR}:${W1_PORT}  -> next=${MAC_ADDR}:${W0_PORT}"
...
# === Validate ===
PASS=false
if [ -n "$GENERATED" ] && [ -z "$ERROR" ] && [ "$WORKERS_CONNECTED" = "2" ]; then
    PASS=true
fi

if [ "$PASS" = true ]; then
    echo ""
    echo "=== CROSS-NODE 2-DOMAIN HTTP API E2E TEST PASSED ==="
    echo "Reports: ${REPORT_DIR}"
    exit 0
else
    echo ""
    echo "=== CROSS-NODE 2-DOMAIN HTTP API E2E TEST FAILED ==="
