...
OUT = SRC.with_name(
    "开题报告_新版8_沈达_面向大模型推理与强化学习后训练的异构算力协同可行性理论与关键技术研究.docx"
)
...
def main():
    ...
    set_text(
        direction_para,
        "基于以上分析，本课题拟以异构负载的分层协同扩展为主线，从可行性建模入手：在推理服务中验证请求级、阶段级与条件性任务内协同的适用边界，在强化学习工作流中验证阶段级承接、异步执行与弹性编排的适用边界，并在确有需要时引入弱中心或去中心化支撑机制，形成面向异构大模型基础设施的可验证技术方案。",
    )
    ...
    for p in doc.paragraphs:
        if "建立设备—网络—任务统一的异构算力协同模型" in p.text:
            ...
            set_text(p, "（2）方法目标：推理线形成容量感知请求路由、阶段放置、HCP 条件性任务内协同和计算—通信联合调度；RL 线形成阶段能力合同、版本时效控制、异构阶段编排、弹性恢复与局部回退方法，并明确集中式、弱中心和多智能体决策的适用边界。")
            ...
            set_text(p, "（4）实验目标：在多代、多厂商或不同设备能力的混合环境中，推理侧对比单设备、同构基线、静态异构路由、动态调度与 HCP 条件性扩展，RL 侧对比固定同位、静态阶段放置、异步流水线与能力合同驱动编排，系统评估完成时间、吞吐、显存、通信、版本时效、扩展性和故障恢复能力。")
        elif "不预设单一固定加速倍数" in p.text:
            set_text(p, "（4）实验目标：在多代、多厂商或不同设备能力的混合环境中，推理侧对比单设备、同构基线、静态异构路由、动态调度与 HCP 条件性扩展，RL 侧对比固定同位、静态阶段放置、异步流水线与能力合同驱动编排，系统评估完成时间、吞吐、显存、通信、版本时效、扩展性和故障恢复能力；不预设单一固定加速倍数，而是报告不同条件下的收益区间和失效边界。")
    ...
    new_refs = [
        ...
        "[39] Prime Intellect. PRIME-DiLoCo: Elastic and Low-Communication Distributed Training[EB/OL]. https://github.com/PrimeIntellect-ai/prime-diloco, 2025.",
    ]
    ...
    replace_with_paragraphs(rl_background, body_template, [
        ...
        "RL线与推理线严格分工：HCP只属于推理线的长上下文Prefill，不参与rollout、奖励评估、策略梯度、参数同步或策略更新。RL线复用既有推理和训练后端，研究如何以阶段能力合同描述设备类型、后端、显存/计算下限、网络依赖、模型版本、输入输出规模和可容忍等待时间，并据此判定不同异构设备能否承接某一阶段。",
        "本课题在RL线拟新增三个可实施模块：构建带模型版本、输入输出规模、资源需求和可容忍滞后的阶段任务图；实现以队列长度、单位有效样本成本、可用显存、通信时间和恢复窗口为约束的阶段放置与并发控制；建立面向worker加入、失效、降级和恢复的状态管理与回退规则。该设计把已有工作流框架的“能运行”进一步变成“在异构资源上可解释地选择阶段，并量化版本时效与弹性代价”的问题。",
    ])
    ...
    replace_with_paragraphs(rl_mid, body_template, [
        "在稳定资源池中，RLBoost指出rollout具有近似无状态、易并行的特征，适合承接抢占式或碎片化资源；策略训练仍需要紧耦合GPU和全互联。因此实验的第一阶段将以HetRL式启发式/ILP或约束优化作为可解释基线，优先把异构性用于rollout、奖励评估和数据处理，策略更新则保留同构或局部同构设备组。该边界避免将“阶段级异构承接”误写成“在弱链路上实现常规TP/DP训练”。",
        "实际对照包括固定同位部署、静态阶段放置、只采用异步流水线，以及能力合同驱动的动态阶段编排。比较中必须固定模型版本、token预算、奖励模型和任务集合，并报告rollout吞吐、单位有效样本成本、样本有效率、策略更新等待时间、版本滞后、资源利用率、网络传输量和任务质量。只有组合机制同时改善有效吞吐或资源利用率且未损害策略质量时，才能说明其为异构RL工作流增加了实际价值；否则应保留更简单的基线。",
    ])
    ...
    # Version 8: HCP is an inference-only innovation.  The RL line is
    # explicitly independent and studies heterogeneous workflow support.
    practical_boundary = next(p for p in doc.paragraphs if p.text.startswith("为使研究内容能够直接落到实现与实验"))
    set_text(
        practical_boundary,
        "为使研究内容能够直接落到实现与实验，课题对两条研究线采用不同的创新归属。vLLM、Dynamo类框架已经提供模型执行、连续批处理、请求路由、Prefill/Decode分离和KV管理等服务层能力；HetRL、Prime RL、Prime DiLoCo分别提供异构阶段调度、异步rollout—训练流水线与弹性低频同步的基线。这些能力将被复用，而不作为本课题的创新。HCP当前原型已经验证了非均匀seq_chunk_len与block_size下的P2P KV ring、逐块online softmax、跨设备控制/数据通路以及vLLM worker/connector原型；其待补齐的准入决策、自动非对称分片、生产框架适配和端到端调度闭环，仅属于推理线。RL线不使用HCP，而是研究不同异构后端的阶段能力约束、异步版本时效与弹性恢复如何共同决定工作流能否获得有效吞吐。",
    )

    rl_heading = next(p for p in doc.paragraphs if p.text.startswith("3.1.2  面向强化学习工作负载"))
    rl_body_template = next(p for p in doc.paragraphs if p.text.startswith("面向强化学习相关工作负载，自适应RLHF放置"))
    rl_content_v8 = [
        "面向强化学习相关工作负载，自适应RLHF放置、HetRL、Prime RL等已提供可复用的工作流基线：将rollout、奖励/价值评估、策略更新和检查点按任务依赖组织，并可采用同步或异步流水线。Prime RL仓库把vLLM作为推理端、FSDP2作为训练端，并由orchestrator串起端到端流程，但当前运行前提仍是至少一块NVIDIA GPU；HetRL则在异构GPU与网络上联合安排阶段放置。这些工作分别说明了工作流解耦、异构调度和异步执行的可行性，却没有直接给出面向跨厂商后端、动态资源和策略时效约束的一体化判定。",
        "RL线的研究对象与HCP严格解耦：HCP只服务于推理线的长上下文Prefill，不进入rollout、奖励评估或策略更新的执行机制。RL线复用既有推理和训练后端，通过描述每个阶段可接受的设备类型、显存/计算下限、网络依赖、模型版本、输入输出规模和可容忍等待时间，建立阶段能力合同。调度器据此决定何种异构设备可承担rollout、奖励/评估或数据处理，何种策略更新必须保留在高带宽同构或局部同构设备组，避免把“阶段可拆分”误写成“所有阶段可跨弱链路任意迁移”。",
        "本课题在RL线的候选增量贡献是把三类已有能力合并为可检验的工作流机制：第一，基于阶段能力合同的跨后端准入和放置，不仅依据峰值算力，还检查模型格式、后端可用性、显存、链路与批处理限制；第二，基于队列长度、策略版本滞后、样本完整性和单位有效样本成本的异步并发控制，防止吞吐提升以陈旧样本或无效更新为代价；第三，基于worker状态的加入、恢复、降级与回退，使动态资源只有在恢复窗口和一致性约束满足时才进入工作流。上述组合需要实验验证，不能预设HetRL、Prime RL或Prime DiLoCo未单独实现过其中某一能力。",
        "在稳定资源池中，先以HetRL式启发式/ILP或约束优化作为可解释基线，比较固定同位部署、静态阶段放置和能力合同驱动的动态阶段编排；在动态资源池中，再与Prime RL式异步流水线、Prime DiLoCo式弹性设备网格和异步检查点分别对照。只有当动态编排同时改善有效rollout吞吐或资源利用率，且版本滞后、样本有效性、策略质量与恢复时间不劣于基线时，才能将其视为异构RL支持的实际价值。",
        "去中心化与多智能体强化学习仍处于可选对照层：前者只在集中式/分层调度的资源发现或局部恢复成为瓶颈时考察，后者只在规则、启发式和约束优化面对局部观测或强非平稳状态时考察。它们的加入必须在相同资源预算下证明决策收益覆盖训练、探索和恢复开销，否则以更简单的基线为最终系统方案。",
    ]
    replace_with_paragraphs(rl_body_template, body_template, rl_content_v8)
    # Remove remaining v7 RL paragraphs, leaving the replacement block.
    for prefix in (
        "与推理线的关系需要严格区分",
        "本课题在RL线拟新增三个可实施模块",
        "在稳定资源池中，RLBoost指出",
        "实际对照包括固定同位部署",
        "在动态资源环境中，DiLoCo与INTELLECT-1证明",
        "去中心化与多智能体强化学习均处于可选对照层",
    ):
        matches = [p for p in doc.paragraphs if p.text.startswith(prefix)]
        if matches:
            remove_para(matches[0])

    rl_plan_heading = next(p for p in doc.paragraphs if p.text.startswith("3.2.2  强化学习工作负载"))
    rl_plan_template = next(p for p in doc.paragraphs if p.text.startswith("第一步，以可回放的LLM-RL阶段任务图"))
    rl_plan_v8 = [
        "第一步，以可回放的LLM-RL阶段任务图实现研究基线，而非直接改写完整训练框架。任务图为rollout、奖励/评估、策略更新和检查点记录模型副本、输入输出规模、计算/显存需求、参数版本和可容忍等待时间；画像同时记录候选设备的后端能力、模型/精度兼容性、显存、网络、队列和恢复窗口。该层可先接入公开工作流的轨迹或小规模开源管线，验证调度逻辑，不把Prime RL或HetRL代码本身视为需要重复开发的对象。",
        "第二步，实现阶段能力合同、放置与并发控制。能力合同首先排除无法运行目标模型或不满足显存/精度要求的设备，再以规则/启发式提供低开销在线决策，以ILP/约束优化提供小规模对照；调度器优先将可弹性扩展的rollout、奖励评估和数据处理放到匹配的异构资源，将高频更新约束在高带宽同构或局部同构设备组。每次放置都同时检查队列长度、参数版本、样本完整性和预计网络时间，并保留不迁移或降低并发度的回退选择。",
        "第三步，验证异构RL支持的增量价值。设置固定同位部署、静态异构阶段放置、只采用异步流水线、能力合同驱动的动态阶段编排四种方案。它们分别对应常规RLHF部署、HetRL式阶段放置、Prime RL式异步工作流和本课题的组合候选。所有方案固定模型版本、token预算、奖励模型、任务集合与资源预算，比较rollout吞吐、单位有效样本成本、策略更新等待时间、版本滞后、样本有效率、资源利用率、网络传输量和任务质量。",
        "第四步，在可控故障注入与弱连接场景验证弹性边界。对节点加入退出、链路降级、检查点恢复和worker失效分别记录重构时间、有效吞吐、参数/样本版本一致性和恢复后策略质量。Prime DiLoCo的ElasticDeviceMesh、异步检查点和live recovery作为弹性基线；本课题研究在何种恢复窗口、队列状态和阶段依赖下允许节点进入、暂停或退出工作流。若弹性机制没有显著减少停机或提高有效资源利用率，便保留集中式/分层方案，而不以“接入更多节点”代替真实收益。",
    ]
    replace_with_paragraphs(rl_plan_template, body_template, rl_plan_v8)
    for prefix in (
        "第二步，实现阶段放置与并发控制",
        "第三步，验证HCP与RL工作流的可组合性",
        "动态调度实验先采用规则、启发式和约束优化",
    ):
        matches = [p for p in doc.paragraphs if p.text.startswith(prefix)]
        if matches:
            remove_para(matches[0])
    # The v7 fourth step has the HCP-specific fallback wording.  Remove that
    # paragraph explicitly so the independent RL elasticity step above stays.
    for p in list(doc.paragraphs):
        if p.text.startswith("第四步，在可控故障注入与弱连接场景验证弹性边界") and "HCP回退" in p.text:
            remove_para(p)
        elif p.text.startswith("第四步，在可控故障注入与弱连接场景验证弹性边界") and "先比较集中式/分层基线" in p.text:
            remove_para(p)
