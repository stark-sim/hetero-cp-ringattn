...
run_driver() { # side label
    ...
    ssh -n -f -o ConnectTimeout=20 "${WHITE_SSH}" "mkdir -p ${state_dir} && setsid bash ~/hetero-cp-ringattn/scripts/phase3_10_kv_wall_${side}_driver.sh ${RUN_ID} > ${state_dir}/driver.log 2>&1 </dev/null"
    local status=""
