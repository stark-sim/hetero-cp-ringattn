#!/bin/bash
# Cross-node 3-domain HTTP API concurrent request end-to-end test.
...
RUN_ID="cross-node-http-concurrent-3domain-$(date +%Y%m%d-%H%M%S)"
...
echo "=== Cross-Node 3-Domain HTTP API Concurrent E2E Test ==="
echo "RUN_ID=${RUN_ID}"
...
if [ "$PASS1" = true ] && [ "$PASS2" = true ]; then
    echo ""
    echo "=== CROSS-NODE 3-DOMAIN CONCURRENT HTTP API E2E TEST PASSED ==="
    echo "Both requests completed successfully."
    echo "Reports: ${REPORT_DIR}"
    exit 0
else
    echo ""
    echo "=== CROSS-NODE 3-DOMAIN CONCURRENT HTTP API E2E TEST FAILED ==="
