...
OUT = BASE / '开题报告_新版19_沈达_面向大模型推理与强化学习后训练的异构算力负载承接研究.docx'
...
def main():
    ...
    p = framing
    state = IMG_DIR / 'heterogeneous-service-positioning-v3.png'
    if state.exists():
        cap = add_figure(doc, p, state, '图4  异构算力服务的承接粒度与本课题定位', width=6.0)
        p = cap
    rows = [
    ...
    overview = IMG_DIR / 'heterogeneous-two-research-directions-overview-v5-labeled.png'
    if overview.exists():
        cap = add_figure(doc, p, overview, '图1  两条独立研究路线的协同粒度与研究边界', width=6.0)
        p = cap
    rows = [
    ...
    matrix = IMG_DIR / 'heterogeneous-value-boundary-v2.png'
    if matrix.exists():
    ...
    rl = IMG_DIR / 'heterogeneous-llm-rl-engineering-composition-v1.png'
    if rl.exists():
        add_figure(doc, p, rl, '图3  LLM-RL后训练设备池的工程组合与运行边界', width=5.9)
