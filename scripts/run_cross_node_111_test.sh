#!/bin/bash
# 111-token cross-node hetero test (Mac MPS + white RTX 4090)
set -euo pipefail

REPORT_DIR="/Users/stark_sim/VSCodeProjects/hetero-cp-ringattn/reports/cross-node-white-111-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$REPORT_DIR"
...
PROMPT_FILE="/tmp/hcp_prompt_111.txt"
