#!/bin/bash
# Local HTTP API concurrent request end-to-end test.
# Launches coordinator (HTTP mode) + 2-domain workers, submits 2 requests
# concurrently, and verifies both complete successfully.
...
echo "=== HTTP API Concurrent Local E2E Test ==="
...
curl -s -X POST http://localhost:${HTTP_PORT}/v1/completions \
    -H "Content-Type: application/json" \
    -d '{"prompt": "Once upon a time", "max_tokens": 5, "temperature": 0.0}' \
    > /tmp/resp2.json &
PID2=$!
...
echo ""
echo "--- Response 1 ---"
cat /tmp/resp1.json | python3 -m json.tool || cat /tmp/resp1.json

echo ""
echo "--- Response 2 ---"
cat /tmp/resp2.json | python3 -m json.tool || cat /tmp/resp2.json

# Extract generated text
GENERATED1=$(python3 -c "import json; d=json.load(open('/tmp/resp1.json')); print(d['choices'][0]['text'].strip())" 2>/dev/null || echo "")
GENERATED2=$(python3 -c "import json; d=json.load(open('/tmp/resp2.json')); print(d['choices'][0]['text'].strip())" 2>/dev/null || echo "")
...
if [ -n "$GENERATED1" ] && [[ "$GENERATED1" != "[error:"* ]]; then
    PASS1=true
fi

if [ -n "$GENERATED2" ] && [[ "$GENERATED2" != "[error:"* ]]; then
    PASS2=true
fi

if [ "$PASS1" = true ] && [ "$PASS2" = true ]; then
    echo ""
    echo "=== CONCURRENT HTTP API E2E TEST PASSED ==="
    echo "Both requests completed successfully."
else
    echo ""
    echo "=== CONCURRENT HTTP API E2E TEST FAILED ==="
    if [ "$PASS1" != true ]; then
        echo "Request 1 failed or returned error"
    fi
    if [ "$PASS2" != true ]; then
        echo "Request 2 failed or returned error"
    fi
    echo "--- Coordinator log ---"
    tail -30 /tmp/hcp_http_coord.log || true
    echo "--- Worker 0 log ---"
