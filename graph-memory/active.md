...
[2026-07-30 next-node analysis] 复核发现单层 runner 仍通过共享作用域提供 residual/normalized hidden。候选下一节点调整为先显式化自足 LayerPacket，并验证 payload 与 context 长度无关；该候选保持 open，待用户确认后才实施。多层 handoff+唯一 logits 顺延。
