...
capacity-aware 连续分片与加权 Striped 是 HCP 的两种候选调度策略。\n\n结论：\n- CPU mock、white CUDA、pearl HIP 三重证据一致表明：在 2-domain 3:1 seq_len=4096 单进程场景下，Striped 把 peer compute 转移到大 domain，使瓶颈更慢，未改善 wall-time。\n- 根据简洁性原则和实测证据，默认调度策略保留为 capacity-aware 连续分片。\n- Striped 实现保留在仓库中作为可选项，未来若 multi-node 真实分布式实验出现反证可重新评估。
...
_updated: 2026-06-29 06:16:16_
### 在真实 multi-node 场景下重新评估 Striped（如未来需要）

type: `task` · status: `pending` · confidence: 0.75 · importance: 0.5

当前单进程 CUDA/HIP 证据已足够做出默认保留连续分片的决策。\n\n若未来要重新挑战该决策，需要在真实 multi-node 异构分布式场景（white + pearl 各跑独立 worker，非单进程 mock）下复跑 1M context 或 4096/3:1 配置，并测量端到端 wall-time、显存占用、P2P/QUIC 传输开销。当前暂无优先级。
