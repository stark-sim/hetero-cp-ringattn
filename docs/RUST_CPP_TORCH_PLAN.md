...
- Linux 下 CUDA 版 libtorch 需要保留 `libtorch_cuda` / `c10_cuda` 这类 registration library；`rust/build.rs` 会在检测到二者时用 `--no-as-needed` 防止链接器把它们优化掉。远端 `ldd rust/target/debug/hcp-ringattn-rust | grep -E 'torch|c10|cuda'` 应能看到 `libtorch_cuda.so` 和 `libc10_cuda.so`。
