...
- `DomainModelState` 是当前最小模型状态结构：每个 domain 持有自己的 Q chunk 和 K/V storage；发送 K/V block 时从 source state 切片，捕获 compute update 时携带 target state 的 Q payload。
...
- Domain model state 验证已通过：`cargo test --offline` 覆盖 state-owned K/V block slicing 和 domain-local Q payload；本机非沙箱 MPS 主 smoke、远端 CUDA 主 smoke、`RUN_ID=rust-remote-cp-modelstate-20260426` 三节点 remote CP smoke 均通过。
- 当前 query chunk bridge 已消费 Rust/domain-side model state 中的 Q payload 和 K/V storage slice；这些 tensor 仍是 deterministic model-state fixtures，尚未升级为真实模型 activation / weight lifecycle。
