...
from hcp_vllm_plugin.ring_backend import stage_peer_kv
...
class HcpRingKvConnector(KVConnectorBase_V1):
    ...
    def get_num_new_matched_tokens(
        ...
        if self._ring_role != "consumer" or self._prefix_len == 0:
            return 0, False
        ...
        for new_req in scheduler_output.scheduled_new_reqs:
            if new_req.req_id in self._requests_need_load:
                ...
                meta.requests.append(
                    RingReqMeta(is_store=False, chunk_key=self._prefix_chunk_ids[0])
                )
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs: Any) -> None:
        """Fetch the peer chunk's per-layer KV into the ring backend's
        TRANSIENT staging dict.  Never writes to the paged pool/block table."""
        metadata = self._get_connector_metadata()
        assert isinstance(metadata, HcpRingConnectorMetadata)
        need = [r for r in metadata.requests if not r.is_store]
        if not need:
            return
        expected = _align_down(self._prefix_len, self._block_size)
        for req in need:
            staged = 0
            total_bytes = 0
            for layer_name, layer in forward_context.no_compile_layers.items():
                ...
                tensors = self._load_layer(req.chunk_key, layer_name, device)
                if tensors is None:
                    raise RuntimeError(
                        "HcpRingKvConnector: peer KV fetch failed for "
                        f"{req.chunk_key}/{layer_name} (peer={self._peer_url})"
                    )
                k, v = tensors
                if k.shape[0] != expected:
                    raise RuntimeError(
                        f"HcpRingKvConnector: staged peer KV length {k.shape[0]} "
                        f"!= expected external prefix {expected} for {layer_name}"
                    )
                stage_peer_kv(layer_name, k, v)
                staged += 1
            ...
            logger.info(
                ...
                expected,
                total_bytes / 2**20,
                self._peer_url or "local store",
            )

    def _load_layer(
        self, chunk_key: str, layer_name: str, device: str
