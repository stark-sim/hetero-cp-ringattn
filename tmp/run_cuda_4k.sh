...
HCP_TORCH_DEVICE=cuda:1 "$BINARY" --distributed-role worker \
    ...
    > /tmp/worker0_4k.log 2>&1 &

HCP_TORCH_DEVICE=cuda:2 "$BINARY" --distributed-role worker \
    --domain-id 1 --seq-offset 2048 --model-dir "$MODEL" \
    --listen-addr 127.0.0.1:9101 --next-peer-addr 127.0.0.1:9100 \
    --coordinator-addr 127.0.0.1:9000 --num-domains 2 \
    > /tmp/worker1_4k.log 2>&1 &

sleep 2

time "$BINARY" --distributed-role coordinator \
    --model-dir "$MODEL" \
    --prompt-file /tmp/prompt_4k.txt \
    --max-tokens 4 --temperature 0.0 --num-domains 2 \
    --listen-addr 127.0.0.1:9000 --chunk-sizes 2048,2048 \
    > /tmp/coord_4k.log 2>&1

echo "=== COORD ==="
cat /tmp/coord_4k.log | tail -10
echo "=== W0 ==="
cat /tmp/worker0_4k.log | tail -5
echo "=== W1 ==="
cat /tmp/worker1_4k.log | tail -5
