...
- **CP 节点双角色**：真正的 remote CP P2P node 应同时具备 inbound listener 和 outbound peer；当前 `tcp_remote_pair` 只是单连接 server/client 握手，不代表最终节点拓扑。
...
- `rust/src/protocol.rs` 定义 Rust 侧可序列化 message schema、本地 P2P queue transport、remote `tcp_remote_pair` transport smoke 和 protocol reports。
...
7. 当前 Rust protocol smoke 以本地 P2P queue transport 验证完整 ring 转发路径，并以 `tcp_remote_pair` 验证双进程 / 双机器 send/recv frame。
