...
- [x] [2026-05-06] **Python Worker SDK Phase 2.5 vLLM 控制面流程验证**（commit `fd8649c` + `417d66d`）：`test_worker_2domain.py` 支持 `--backend vllm`，`LinkedMockKvTransport` 通过 multiprocessing Queue 在进程间交换 KV。参考 tokens 改用 transformers 计算（vLLM `LLM` API 不暴露 KV cache）。单卡双 vLLM 实例 OOM 不可行（vLLM `gpu_memory_utilization` 基于总显存比例，首个实例加载后第二个实例 `kv_cache_size` 为负，即使 0.15 仍失败）。Phase 2.5 目标确认为控制面流程验证，真实 KV ring 需 Phase 3 接入 vLLM `LLMEngine` 底层 API 或多卡环境。
